export const translations = {
  pt: {
    // Navbar
    nav: {
      home: "Início",
      pricing: "Preços",
      dashboard: "Painel",
      signIn: "Entrar",
      startFreeTrial: "Começar grátis",
    },
    // Hero
    hero: {
      badge: "Economize até 30% na sua conta de energia",
      title: "Controle inteligente do seu",
      titleHighlight: "consumo de energia",
      description: "O Pulse ajuda você a monitorar, analisar e otimizar o uso de energia da sua casa. Economize dinheiro enquanto reduz seu impacto ambiental.",
      cta: "Comece seu teste grátis",
      secondaryCta: "Ver planos",
      noCreditCard: "Sem cartão de crédito • Teste grátis de 14 dias",
      stats: {
        users: "10k+ utilizadores",
        rating: "4.9/5 avaliação",
        savings: "Economia de €2M+",
      },
    },
    // Features
    features: {
      badge: "Funcionalidades",
      title: "Tudo que você precisa para economizar energia",
      description: "Ferramentas poderosas para ajudar você a entender e reduzir seu consumo de energia.",
      items: [
        {
          title: "Monitoramento em Tempo Real",
          description: "Acompanhe seu uso de energia em tempo real com atualizações ao vivo e insights detalhados.",
        },
        {
          title: "Orçamento Inteligente",
          description: "Defina metas de orçamento de energia e receba alertas antes de ultrapassar seus limites.",
        },
        {
          title: "Análise de Consumo",
          description: "Entenda seus padrões de uso com análises detalhadas e recomendações personalizadas.",
        },
        {
          title: "Insights Familiares",
          description: "Compare o uso de energia de todos na casa e identifique oportunidades de economia.",
        },
        {
          title: "Comparação de Tarifas",
          description: "Encontre os melhores planos de energia para seu padrão de consumo.",
        },
        {
          title: "Impacto Ambiental",
          description: "Veja como suas economias de energia contribuem para um planeta mais sustentável.",
        },
      ],
    },
    // Social Proof
    socialProof: {
      badge: "Depoimentos",
      title: "Amado por milhares de usuários",
      description: "Veja o que nossa comunidade diz sobre a experiência com o Pulse.",
      testimonials: [
        {
          name: "Maria S.",
          role: "Proprietária",
          content: "O Pulse me ajudou a reduzir minha conta de energia em 30% em apenas 3 meses. Os insights são incrivelmente práticos.",
        },
        {
          name: "João K.",
          role: "Inquilino",
          content: "Finalmente um app que torna o rastreamento de energia simples. Adoro a função de alertas de orçamento!",
        },
        {
          name: "Ana R.",
          role: "Família de 4",
          content: "Economizámos mais de €250 este ano a usar o Pulse. As crianças até gostam de ver as nossas estatísticas diárias!",
        },
      ],
    },
    // CTA
    cta: {
      title: "Pronto para começar a economizar?",
      description: "Junte-se a milhares de usuários que já estão reduzindo seus custos de energia e impacto ambiental com o Pulse.",
      primaryCta: "Comece seu teste grátis",
      secondaryCta: "Comparar planos",
      noCreditCard: "Sem cartão de crédito • Teste grátis de 14 dias",
    },
    // Footer
    footer: {
      description: "Monitoramento inteligente de energia para casas modernas. Economize dinheiro enquanto salva o planeta.",
      product: "Produto",
      features: "Funcionalidades",
      pricing: "Preços",
      dashboard: "Painel",
      company: "Empresa",
      about: "Sobre",
      blog: "Blog",
      careers: "Carreiras",
      support: "Suporte",
      helpCenter: "Central de Ajuda",
      contact: "Contato",
      privacy: "Privacidade",
      copyright: "© 2024 Pulse. Todos os direitos reservados.",
    },
    // Pricing
    pricing: {
      badge: "Preços",
      title: "Escolha o plano certo para você",
      description: "Comece grátis e faça upgrade quando precisar. Todos os planos incluem um teste grátis de 14 dias.",
      monthly: "Mensal",
      yearly: "Anual",
      popular: "Popular",
      perMonth: "/mês",
      startTrial: "Começar teste grátis",
      getStarted: "Começar agora",
      contactSales: "Falar com vendas",
      savePercent: "Economize 20%",
      plans: {
        free: {
          name: "Grátis",
          description: "Perfeito para começar",
          features: [
            "Monitoramento básico de energia",
            "Relatórios semanais",
            "1 dispositivo conectado",
            "Histórico de 30 dias",
          ],
        },
        pro: {
          name: "Pro",
          description: "Para usuários comprometidos",
          features: [
            "Tudo do Grátis",
            "Monitoramento em tempo real",
            "Dispositivos ilimitados",
            "Alertas de orçamento",
            "Relatórios avançados",
            "Suporte prioritário",
          ],
        },
        family: {
          name: "Família",
          description: "Para toda a casa",
          features: [
            "Tudo do Pro",
            "Até 6 membros",
            "Insights familiares",
            "Comparação de tarifas",
            "Gerente de conta dedicado",
            "Acesso à API",
          ],
        },
      },
    },
    // Onboarding
    onboarding: {
      step: "Etapa",
      of: "de",
      next: "Próximo",
      previous: "Anterior",
      finish: "Concluir Configuração",
      steps: {
        goals: {
          title: "Quais são seus objetivos?",
          description: "Selecione o que você deseja alcançar com o Pulse.",
          options: [
            { id: "save-money", label: "Economizar dinheiro", description: "Reduzir contas de energia mensais" },
            { id: "track-usage", label: "Acompanhar uso", description: "Monitorar consumo de energia" },
            { id: "go-green", label: "Ser sustentável", description: "Reduzir pegada de carbono" },
            { id: "budget", label: "Gerenciar orçamento", description: "Manter gastos sob controle" },
          ],
        },
        budget: {
          title: "Qual é o seu orçamento mensal de energia?",
          description: "Isso ajuda-nos a definir metas e alertas realistas.",
          placeholder: "Ex: € 100",
          hint: "Pode ajustar isto depois nas configurações.",
        },
        household: {
          title: "Quantas pessoas moram na sua casa?",
          description: "Isso nos ajuda a fornecer melhores comparações e insights.",
          options: ["1 pessoa", "2 pessoas", "3-4 pessoas", "5+ pessoas"],
        },
      },
      planRecommendation: {
        title: "Plano Recomendado para Você",
        description: "Com base nas suas respostas, recomendamos:",
        based: "Baseado no seu perfil:",
        startWith: "Começar com",
        viewAllPlans: "Ver todos os planos",
      },
      success: {
        title: "Tudo pronto!",
        description: "Seu painel está pronto. Vamos começar a economizar energia!",
        cta: "Ir para o Painel",
      },
    },
    // Dashboard
    dashboard: {
      title: "Painel",
      welcome: "Bem-vindo de volta!",
      welcomeUser: "Bem-vindo de volta,",
      subtitle: "Aqui está sua visão geral de energia.",
      menu: {
        overview: "Visão Geral",
        analytics: "Análises",
        devices: "Dispositivos",
        alerts: "Alertas",
        settings: "Configurações",
      },
      kpi: {
        consumption: {
          title: "Consumo Mensal",
          unit: "kWh",
          change: "vs. mês passado",
        },
        cost: {
          title: "Custo Estimado",
          change: "vs. mês passado",
        },
        savings: {
          title: "Economia",
          change: "da sua meta",
        },
        budget: {
          title: "Status do Orçamento",
          status: {
            onTrack: "No caminho certo",
            overBudget: "Acima do orçamento",
          },
        },
      },
      charts: {
        daily: "Uso Diário",
        monthly: "Tendência Mensal",
      },
      loading: "Carregando dados...",
      error: {
        title: "Erro ao carregar dados",
        description: "Não foi possível buscar seus dados de energia. Tente novamente.",
        retry: "Tentar novamente",
      },
      empty: {
        title: "Nenhum dado ainda",
        description: "Comece a rastrear seu uso de energia para ver insights aqui.",
        cta: "Adicionar primeiro dispositivo",
      },
      signOut: "Sair",
      impact: {
        title: "Impacto Ambiental",
        subtitle: "O seu contributo para um planeta mais verde",
        co2Avoided: "CO₂ Evitado",
        thisYear: "Este ano",
        treesEquivalent: "Árvores Equivalentes",
        planted: "Plantadas",
        kmNotDriven: "Km Não Percorridos",
        byCar: "De carro",
        waterSaved: "Água Poupada",
        indirectly: "Indiretamente",
        monthlyCo2Reduction: "Redução de CO₂ Mensal",
        energyMix: "Mix Energético",
        ecoHero: "Eco-Herói do Mês! 🌱",
        ecoHeroDescription: "Parabéns! Está entre os 10% de utilizadores com maior redução de pegada de carbono.",
        nationalRanking: "Ranking Nacional",
      },
      settings: {
        profile: "Perfil",
        notifications: "Notificações",
        security: "Segurança",
        billing: "Faturação",
        help: "Ajuda",
        updateProfile: "Atualizar Perfil",
        profileUpdated: "Perfil atualizado com sucesso!",
        updateError: "Erro ao atualizar perfil",
        paymentMethod: "Método de Pagamento",
        noPaymentMethod: "Nenhum método configurado",
        addCard: "Adicione um cartão para fazer upgrade",
        freePlan: "Plano gratuito com funcionalidades básicas",
        premiumPlan: "Acesso a todas as funcionalidades premium",
        upgrade: "Fazer upgrade",
        helpCenter: "Centro de Ajuda",
        faq: "Perguntas frequentes",
        faqDesc: "Respostas às dúvidas mais comuns",
        usageGuide: "Guia de utilização",
        usageGuideDesc: "Aprenda a usar todas as funcionalidades",
        contactSupport: "Contactar suporte",
        contactSupportDesc: "Fale com a nossa equipa",
        reportIssue: "Reportar problema",
        reportIssueDesc: "Informe-nos sobre bugs ou erros",
      },
      alerts: {
        unreadAlerts: "alertas por ler",
        noNewAlerts: "Nenhum alerta novo",
      },
      rewards: {
        title: "Recompensas",
        subtitle: "Acompanhe o seu progresso e conquistas",
        points: "Pontos de Eficiência",
        level: "Nível",
        tier: "Nível",
        pointsToLevel: "pontos para o próximo nível",
        streak: "Sequência Atual",
        days: "dias",
        streakDesc: "Dias consecutivos abaixo da meta de consumo",
        badgesEarned: "Badges Conquistadas",
        keepGoing: "Continue assim para desbloquear mais!",
        monthlyChallenges: "Desafios Mensais",
        challengesSubtitle: "Ative desafios e ganhe pontos extra",
        until: "Até",
        unlockedBadges: "Badges Desbloqueadas",
        yourAchievements: "As suas conquistas",
        lockedBadges: "Por Desbloquear",
        howItWorks: "Como funcionam os pontos?",
        reduceConsumption: "Reduza o consumo",
        perPercent: "por cada % reduzido",
        stayBudget: "Fique no orçamento",
        perDay: "por dia abaixo da meta",
        completeChallenges: "Complete desafios",
        bonusPoints: "Bónus variável por desafio",
        // Program Advantages
        programAdvantages: "Vantagens do Programa",
        whyParticipate: "Porque vale a pena participar",
        billDiscounts: "Descontos na Fatura",
        billDiscountsDesc: "Troque pontos por descontos diretos na sua conta de eletricidade.",
        partnerVouchers: "Vouchers de Parceiros",
        partnerVouchersDesc: "Use pontos em supermercados, cafés e lojas de eletrodomésticos.",
        ecoImpact: "Impacto Ambiental",
        ecoImpactDesc: "Contribua para plantar árvores e apoiar energia renovável.",
        // Rewards Catalog
        rewardsCatalog: "Catálogo de Recompensas",
        redeemPoints: "Troque os seus pontos por recompensas reais",
        available: "disponíveis",
        discountsCategory: "Descontos na Fatura",
        vouchersCategory: "Vouchers de Parceiros",
        ecoCategory: "Impacto Ambiental",
        experiencesCategory: "Experiências Exclusivas",
        redeem: "Trocar",
        missing: "em falta",
        rewardRedeemed: "Recompensa resgatada!",
        checkEmail: "Verifique o seu email para mais detalhes.",
        insufficientPoints: "Pontos insuficientes",
        needMorePoints: "Continue a poupar para desbloquear esta recompensa.",
        // Program Rules
        programRules: "Regras do Programa",
        howToEarnAndSpend: "Como ganhar e usar os seus pontos",
        howToEarn: "Como Ganhar Pontos",
        stayOnBudget: "Manter-se no orçamento",
        offPeakUsage: "Consumo fora de pico",
        referFriends: "Referir amigos",
        levelsAndBenefits: "Níveis e Benefícios",
        basicAccess: "Acesso básico",
        inRewards: "em recompensas",
        exclusive: "exclusivos",
        events: "eventos",
        termsAndConditions: "Condições e Validade",
        pointsExpiry: "Pontos expiram após 12 meses de inatividade",
        minimumRedeem: "Mínimo de 200 pontos para trocar recompensas",
        voucherValidity: "Vouchers válidos por 6 meses após resgate",
        nonTransferable: "Pontos e recompensas são pessoais e intransferíveis",
        // B2B Section
        forEnergyCompanies: "Para Empresas de Energia",
        b2bSubtitle: "Implemente gamificação na sua empresa",
        roiTitle: "ROI Comprovado",
        roiDesc: "8-15% redução em picos",
        loyaltyTitle: "Fidelização",
        loyaltyDesc: "+40% retenção de clientes",
        esgTitle: "Metas ESG",
        esgDesc: "Relatórios de sustentabilidade",
        b2bDescription: "Solução white-label com API completa para integração com sistemas de faturação existentes. Personalização total da marca e recompensas.",
        contactSales: "Contactar Vendas",
        downloadBrochure: "Descarregar Brochura",
        badges: {
          firstSavings: "Primeira Poupança",
          firstSavingsDesc: "Poupou energia pela primeira vez",
          budgetMaster: "Mestre do Orçamento",
          budgetMasterDesc: "3 meses consecutivos abaixo do orçamento",
          efficiencyPro: "Eficiência Pro",
          efficiencyProDesc: "Reduziu 20% do consumo mensal",
          streak7: "Semana Verde",
          streak7Desc: "7 dias seguidos abaixo da meta",
          ecoWarrior: "Guerreiro Eco",
          ecoWarriorDesc: "Evitou 100kg de CO₂ num ano",
          yearlyChampion: "Campeão Anual",
          yearlyChampionDesc: "Melhor desempenho do ano",
        },
        challenges: {
          reduce5: "Redutor de 5%",
          reduce5Desc: "Reduzir o consumo em 5% este mês",
          budgetWeek: "Semana Económica",
          budgetWeekDesc: "Ficar abaixo do orçamento diário por 7 dias",
          offPeak: "Herói Fora de Pico",
          offPeakDesc: "80% do consumo em horário económico",
        },
      },
    },
    // Auth
    auth: {
      welcomeBack: "Bem-vindo de volta",
      loginSubtitle: "Entre na sua conta para continuar",
      createYourAccount: "Crie sua conta",
      signupStep1Subtitle: "Comece com seu nome e email",
      completeYourProfile: "Complete seu perfil",
      signupStep2Subtitle: "Defina sua senha e aceite os termos",
      verifyEmail: "Verifique seu email",
      otpSentTo: "Enviamos um código de 6 dígitos para",
      brandingTitle: "Economize energia, economize dinheiro",
      brandingDescription: "Junte-se a milhares de usuários que já estão controlando seu consumo de energia de forma inteligente.",
      savingsAverage: "economia média",
      orContinueWith: "ou continue com email",
      email: "Email",
      emailPlaceholder: "seu@email.com",
      password: "Senha",
      passwordPlaceholder: "Mínimo 6 caracteres",
      passwordHint: "Mínimo de 6 caracteres",
      confirmPassword: "Confirmar senha",
      confirmPasswordPlaceholder: "Digite a senha novamente",
      fullName: "Nome completo",
      fullNamePlaceholder: "Seu nome completo",
      phone: "Telemóvel",
      phonePlaceholder: "912 345 678",
      optional: "opcional",
      forgotPassword: "Esqueceu a senha?",
      signIn: "Entrar",
      createAccount: "Criar conta",
      continue: "Continuar",
      verify: "Verificar",
      back: "Voltar",
      noAccount: "Não tem uma conta?",
      hasAccount: "Já tem uma conta?",
      acceptTermsText: "Eu aceito os",
      termsOfService: "Termos de Serviço",
      and: "e a",
      privacyPolicy: "Política de Privacidade",
      didntReceiveCode: "Não recebeu o código?",
      resendCode: "Reenviar",
      signupSuccess: "Conta criada com sucesso!",
      welcomeMessage: "Bem-vindo ao Pulse. Vamos configurar seu perfil.",
      errors: {
        invalidEmail: "Por favor, insira um email válido",
        invalidOTP: "Por favor, insira o código de 6 dígitos",
        loginFailed: "Falha ao entrar",
        signupFailed: "Falha ao criar conta",
        socialLoginFailed: "Falha no login social",
        emailExists: "Este email já está em uso",
        nameRequired: "Por favor, insira seu nome",
        passwordTooShort: "A senha deve ter no mínimo 6 caracteres",
        passwordMismatch: "As senhas não coincidem",
        acceptTerms: "Você deve aceitar os termos para continuar",
      },
    },
  },
  en: {
    // Navbar
    nav: {
      home: "Home",
      pricing: "Pricing",
      dashboard: "Dashboard",
      signIn: "Sign in",
      startFreeTrial: "Start free trial",
    },
    // Hero
    hero: {
      badge: "Save up to 30% on your energy bill",
      title: "Smart control of your",
      titleHighlight: "energy consumption",
      description: "Pulse helps you monitor, analyze, and optimize your home energy usage. Save money while reducing your environmental impact.",
      cta: "Start your free trial",
      secondaryCta: "View plans",
      noCreditCard: "No credit card required • 14-day free trial",
      stats: {
        users: "10k+ users",
        rating: "4.9/5 rating",
        savings: "£2M+ saved",
      },
    },
    // Features
    features: {
      badge: "Features",
      title: "Everything you need to save energy",
      description: "Powerful tools to help you understand and reduce your energy consumption.",
      items: [
        {
          title: "Real-Time Monitoring",
          description: "Track your energy usage in real-time with live updates and detailed insights.",
        },
        {
          title: "Smart Budgeting",
          description: "Set energy budget goals and receive alerts before you exceed your limits.",
        },
        {
          title: "Usage Analytics",
          description: "Understand your usage patterns with detailed analytics and personalized recommendations.",
        },
        {
          title: "Family Insights",
          description: "Compare energy usage across your household and identify saving opportunities.",
        },
        {
          title: "Rate Comparison",
          description: "Find the best energy plans for your consumption pattern.",
        },
        {
          title: "Environmental Impact",
          description: "See how your energy savings contribute to a more sustainable planet.",
        },
      ],
    },
    // Social Proof
    socialProof: {
      badge: "Testimonials",
      title: "Loved by thousands of users",
      description: "See what our community has to say about their experience with Pulse.",
      testimonials: [
        {
          name: "Sarah M.",
          role: "Homeowner",
          content: "Pulse helped me reduce my energy bill by 30% in just 3 months. The insights are incredibly actionable.",
        },
        {
          name: "James K.",
          role: "Apartment Renter",
          content: "Finally an app that makes energy tracking simple. I love the budget alerts feature!",
        },
        {
          name: "Emily R.",
          role: "Family of 4",
          content: "We've saved over £500 this year using Pulse. The kids even enjoy checking our daily stats!",
        },
      ],
    },
    // CTA
    cta: {
      title: "Ready to start saving?",
      description: "Join thousands of users who are already reducing their energy costs and environmental impact with Pulse.",
      primaryCta: "Start your free trial",
      secondaryCta: "Compare plans",
      noCreditCard: "No credit card required • 14-day free trial",
    },
    // Footer
    footer: {
      description: "Smart energy monitoring for modern homes. Save money while saving the planet.",
      product: "Product",
      features: "Features",
      pricing: "Pricing",
      dashboard: "Dashboard",
      company: "Company",
      about: "About",
      blog: "Blog",
      careers: "Careers",
      support: "Support",
      helpCenter: "Help Center",
      contact: "Contact",
      privacy: "Privacy",
      copyright: "© 2024 Pulse. All rights reserved.",
    },
    // Pricing
    pricing: {
      badge: "Pricing",
      title: "Choose the right plan for you",
      description: "Start free and upgrade when you need to. All plans include a 14-day free trial.",
      monthly: "Monthly",
      yearly: "Yearly",
      popular: "Popular",
      perMonth: "/month",
      startTrial: "Start free trial",
      getStarted: "Get started",
      contactSales: "Contact sales",
      savePercent: "Save 20%",
      plans: {
        free: {
          name: "Free",
          description: "Perfect for getting started",
          features: [
            "Basic energy monitoring",
            "Weekly reports",
            "1 connected device",
            "30-day history",
          ],
        },
        pro: {
          name: "Pro",
          description: "For committed users",
          features: [
            "Everything in Free",
            "Real-time monitoring",
            "Unlimited devices",
            "Budget alerts",
            "Advanced reports",
            "Priority support",
          ],
        },
        family: {
          name: "Family",
          description: "For the whole house",
          features: [
            "Everything in Pro",
            "Up to 6 members",
            "Family insights",
            "Rate comparison",
            "Dedicated account manager",
            "API access",
          ],
        },
      },
    },
    // Onboarding
    onboarding: {
      step: "Step",
      of: "of",
      next: "Next",
      previous: "Previous",
      finish: "Complete Setup",
      steps: {
        goals: {
          title: "What are your goals?",
          description: "Select what you want to achieve with Pulse.",
          options: [
            { id: "save-money", label: "Save money", description: "Reduce monthly energy bills" },
            { id: "track-usage", label: "Track usage", description: "Monitor energy consumption" },
            { id: "go-green", label: "Go green", description: "Reduce carbon footprint" },
            { id: "budget", label: "Manage budget", description: "Keep spending under control" },
          ],
        },
        budget: {
          title: "What's your monthly energy budget?",
          description: "This helps us set realistic goals and alerts.",
          placeholder: "e.g. £100",
          hint: "You can adjust this later in settings.",
        },
        household: {
          title: "How many people live in your home?",
          description: "This helps us provide better comparisons and insights.",
          options: ["1 person", "2 people", "3-4 people", "5+ people"],
        },
      },
      planRecommendation: {
        title: "Recommended Plan for You",
        description: "Based on your answers, we recommend:",
        based: "Based on your profile:",
        startWith: "Start with",
        viewAllPlans: "View all plans",
      },
      success: {
        title: "You're all set!",
        description: "Your dashboard is ready. Let's start saving energy!",
        cta: "Go to Dashboard",
      },
    },
    // Dashboard
    dashboard: {
      title: "Dashboard",
      welcome: "Welcome back!",
      welcomeUser: "Welcome back,",
      subtitle: "Here's your energy overview.",
      menu: {
        overview: "Overview",
        analytics: "Analytics",
        devices: "Devices",
        alerts: "Alerts",
        settings: "Settings",
      },
      kpi: {
        consumption: {
          title: "Monthly Consumption",
          unit: "kWh",
          change: "vs. last month",
        },
        cost: {
          title: "Estimated Cost",
          change: "vs. last month",
        },
        savings: {
          title: "Savings",
          change: "of your goal",
        },
        budget: {
          title: "Budget Status",
          status: {
            onTrack: "On track",
            overBudget: "Over budget",
          },
        },
      },
      charts: {
        daily: "Daily Usage",
        monthly: "Monthly Trend",
      },
      loading: "Loading data...",
      error: {
        title: "Error loading data",
        description: "Unable to fetch your energy data. Please try again.",
        retry: "Try again",
      },
      empty: {
        title: "No data yet",
        description: "Start tracking your energy usage to see insights here.",
        cta: "Add first device",
      },
      signOut: "Sign out",
      impact: {
        title: "Environmental Impact",
        subtitle: "Your contribution to a greener planet",
        co2Avoided: "CO₂ Avoided",
        thisYear: "This year",
        treesEquivalent: "Trees Equivalent",
        planted: "Planted",
        kmNotDriven: "Km Not Driven",
        byCar: "By car",
        waterSaved: "Water Saved",
        indirectly: "Indirectly",
        monthlyCo2Reduction: "Monthly CO₂ Reduction",
        energyMix: "Energy Mix",
        ecoHero: "Eco Hero of the Month! 🌱",
        ecoHeroDescription: "Congratulations! You're among the top 10% of users with the highest carbon footprint reduction.",
        nationalRanking: "National Ranking",
      },
      settings: {
        profile: "Profile",
        notifications: "Notifications",
        security: "Security",
        billing: "Billing",
        help: "Help",
        updateProfile: "Update Profile",
        profileUpdated: "Profile updated successfully!",
        updateError: "Error updating profile",
        paymentMethod: "Payment Method",
        noPaymentMethod: "No payment method configured",
        addCard: "Add a card to upgrade",
        freePlan: "Free plan with basic features",
        premiumPlan: "Access to all premium features",
        upgrade: "Upgrade",
        helpCenter: "Help Center",
        faq: "Frequently Asked Questions",
        faqDesc: "Answers to the most common questions",
        usageGuide: "Usage Guide",
        usageGuideDesc: "Learn how to use all features",
        contactSupport: "Contact Support",
        contactSupportDesc: "Talk to our team",
        reportIssue: "Report Issue",
        reportIssueDesc: "Let us know about bugs or errors",
      },
      alerts: {
        unreadAlerts: "unread alerts",
        noNewAlerts: "No new alerts",
      },
      rewards: {
        title: "Rewards",
        subtitle: "Track your progress and achievements",
        points: "Efficiency Points",
        level: "Level",
        tier: "Tier",
        pointsToLevel: "points to next level",
        streak: "Current Streak",
        days: "days",
        streakDesc: "Consecutive days below consumption goal",
        badgesEarned: "Badges Earned",
        keepGoing: "Keep going to unlock more!",
        monthlyChallenges: "Monthly Challenges",
        challengesSubtitle: "Activate challenges and earn bonus points",
        until: "Until",
        unlockedBadges: "Unlocked Badges",
        yourAchievements: "Your achievements",
        lockedBadges: "Locked",
        howItWorks: "How do points work?",
        reduceConsumption: "Reduce consumption",
        perPercent: "per % reduced",
        stayBudget: "Stay on budget",
        perDay: "per day below goal",
        completeChallenges: "Complete challenges",
        bonusPoints: "Variable bonus per challenge",
        // Program Advantages
        programAdvantages: "Program Advantages",
        whyParticipate: "Why it's worth participating",
        billDiscounts: "Bill Discounts",
        billDiscountsDesc: "Exchange points for direct discounts on your electricity bill.",
        partnerVouchers: "Partner Vouchers",
        partnerVouchersDesc: "Use points at supermarkets, cafes, and appliance stores.",
        ecoImpact: "Environmental Impact",
        ecoImpactDesc: "Contribute to planting trees and supporting renewable energy.",
        // Rewards Catalog
        rewardsCatalog: "Rewards Catalog",
        redeemPoints: "Exchange your points for real rewards",
        available: "available",
        discountsCategory: "Bill Discounts",
        vouchersCategory: "Partner Vouchers",
        ecoCategory: "Environmental Impact",
        experiencesCategory: "Exclusive Experiences",
        redeem: "Redeem",
        missing: "missing",
        rewardRedeemed: "Reward redeemed!",
        checkEmail: "Check your email for more details.",
        insufficientPoints: "Insufficient points",
        needMorePoints: "Keep saving to unlock this reward.",
        // Program Rules
        programRules: "Program Rules",
        howToEarnAndSpend: "How to earn and use your points",
        howToEarn: "How to Earn Points",
        stayOnBudget: "Stay on budget",
        offPeakUsage: "Off-peak usage",
        referFriends: "Refer friends",
        levelsAndBenefits: "Levels and Benefits",
        basicAccess: "Basic access",
        inRewards: "in rewards",
        exclusive: "exclusive",
        events: "events",
        termsAndConditions: "Terms and Conditions",
        pointsExpiry: "Points expire after 12 months of inactivity",
        minimumRedeem: "Minimum of 200 points to redeem rewards",
        voucherValidity: "Vouchers valid for 6 months after redemption",
        nonTransferable: "Points and rewards are personal and non-transferable",
        // B2B Section
        forEnergyCompanies: "For Energy Companies",
        b2bSubtitle: "Implement gamification in your company",
        roiTitle: "Proven ROI",
        roiDesc: "8-15% peak reduction",
        loyaltyTitle: "Customer Loyalty",
        loyaltyDesc: "+40% customer retention",
        esgTitle: "ESG Goals",
        esgDesc: "Sustainability reports",
        b2bDescription: "White-label solution with complete API for integration with existing billing systems. Full brand and rewards customization.",
        contactSales: "Contact Sales",
        downloadBrochure: "Download Brochure",
        badges: {
          firstSavings: "First Savings",
          firstSavingsDesc: "Saved energy for the first time",
          budgetMaster: "Budget Master",
          budgetMasterDesc: "3 consecutive months under budget",
          efficiencyPro: "Efficiency Pro",
          efficiencyProDesc: "Reduced monthly consumption by 20%",
          streak7: "Green Week",
          streak7Desc: "7 days straight below target",
          ecoWarrior: "Eco Warrior",
          ecoWarriorDesc: "Avoided 100kg of CO₂ in a year",
          yearlyChampion: "Yearly Champion",
          yearlyChampionDesc: "Best performance of the year",
        },
        challenges: {
          reduce5: "5% Reducer",
          reduce5Desc: "Reduce consumption by 5% this month",
          budgetWeek: "Budget Week",
          budgetWeekDesc: "Stay below daily budget for 7 days",
          offPeak: "Off-Peak Hero",
          offPeakDesc: "80% of consumption during off-peak hours",
        },
      },
    },
    // Auth
    auth: {
      welcomeBack: "Welcome back",
      loginSubtitle: "Sign in to your account to continue",
      createYourAccount: "Create your account",
      signupStep1Subtitle: "Start with your name and email",
      completeYourProfile: "Complete your profile",
      signupStep2Subtitle: "Set your password and accept the terms",
      verifyEmail: "Verify your email",
      otpSentTo: "We sent a 6-digit code to",
      brandingTitle: "Save energy, save money",
      brandingDescription: "Join thousands of users who are already controlling their energy consumption intelligently.",
      savingsAverage: "average savings",
      orContinueWith: "or continue with email",
      email: "Email",
      emailPlaceholder: "your@email.com",
      password: "Password",
      passwordPlaceholder: "Minimum 6 characters",
      passwordHint: "Minimum 6 characters",
      confirmPassword: "Confirm password",
      confirmPasswordPlaceholder: "Enter password again",
      fullName: "Full name",
      fullNamePlaceholder: "Your full name",
      phone: "Phone",
      phonePlaceholder: "912 345 678",
      optional: "optional",
      forgotPassword: "Forgot password?",
      signIn: "Sign in",
      createAccount: "Create account",
      continue: "Continue",
      verify: "Verify",
      back: "Back",
      noAccount: "Don't have an account?",
      hasAccount: "Already have an account?",
      acceptTermsText: "I accept the",
      termsOfService: "Terms of Service",
      and: "and the",
      privacyPolicy: "Privacy Policy",
      didntReceiveCode: "Didn't receive the code?",
      resendCode: "Resend",
      signupSuccess: "Account created successfully!",
      welcomeMessage: "Welcome to Pulse. Let's set up your profile.",
      errors: {
        invalidEmail: "Please enter a valid email",
        invalidOTP: "Please enter the 6-digit code",
        loginFailed: "Failed to sign in",
        signupFailed: "Failed to create account",
        socialLoginFailed: "Social login failed",
        emailExists: "This email is already in use",
        nameRequired: "Please enter your name",
        passwordTooShort: "Password must be at least 6 characters",
        passwordMismatch: "Passwords do not match",
        acceptTerms: "You must accept the terms to continue",
      },
    },
  },
} as const;

export type Language = keyof typeof translations;
export type Translations = (typeof translations)[Language];
